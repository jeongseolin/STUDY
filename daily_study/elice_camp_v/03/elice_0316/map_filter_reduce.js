const arrayFunctions = {
  map(array, func) {
    ...
    for (let i = 0; i < array.length; i++) {
        newArray.push(func(array[i])); //func = (item) => item * 2
    }
  ...
  filter(array, func) {
    ...
    // func = item => item < 2 // predicate, boolean(true/false) 

    let newArray = [];
    for (let i=0; i < array.length; i++) {
        if (func(array[i])) newArray.push(array[i]); // func = (item) => item < 2
    }
    return newArray;
  },
  ...
  reduce(array, func, initialValue) {
    ...
    for (let i = 0; i < array.length; i++) {
        result = func(result, array[i]); // func = (left, right) => left + right
    }
    return result;
  },
};
