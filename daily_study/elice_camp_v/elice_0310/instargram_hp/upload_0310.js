          ...
          request.onsuccess = function () {
            ...
            store.add(data).onsuccess = function () {
              store.getAll().onsuccess = function (e) {
                ...
                for (let i = 0; i < response.length; i++) { //main for문 그대로 붙여넣은것!
  ...
  if (window.indexedDB) {
    ...
    request.onsuccess = function () {
      const store = request.result
        ...
        .objectStore('posts');
      store.getAll().onsuccess = function (e) {
        const response = e.target.result; // [ {content, image}, {}, {}, ... ]
        
        if (response.length !== 0) { //response 데이터가 하나라도 있다면 다음을 실행해라.
            document.querySelector(".main__posts").innerHTML = ""; //이미지를 넣을때 기존에 있던 데이터는 들어가면 안되므로, 비워준다. 
            for (let i = 0; i < response.length; i++) {
                const postListEl = document.createElement("img");
                postListEl.setAttribute("src", response[i].image);

                document.querySelector(".main__posts").appendChild(postListEl); //이미지가 main__posts 아래에 차곡차곡 저장된다.
            }
        } else {
