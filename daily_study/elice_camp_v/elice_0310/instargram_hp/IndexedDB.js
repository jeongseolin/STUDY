  ...
  fileEl.addEventListener('input', function () {
    ...
    reader.onload = function () {
      ...
      shareBtn.addEventListener("click", function(){
          if (window.indexedDB) { //윈도우내에 indexedDB가 있을 경우에만 실행한다.
            ...
            request.onupgradeneeded = function () { //indexedDB가 버전이 아예 없었다가 새로 생길때, 버전이 바뀔때 마다 실행되는 것 = onupgradeneeded 프로퍼티
            //프로퍼티가 실행될 때, 만들어둔 함수가 실행된다. 
                request.result.createObjectStore("posts", { autoIncrement: true }) //자동으로 1부터 만들어지게 설정
            };
