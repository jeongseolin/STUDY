  ...
  .createServer(function (req, res) {
    ...
    //1.요청 방식이 POST일 경우에 index.html 파일을 웹에 띄워주세요. (true를 수정하세요.)
      ...
      req.on("data", function (chunk) {
        ...
        var data = querystring.parse(chunk.toString()); // querystring.parse()는 url query string을 분석하여 객체로 반환해줍니다.
        res.writeHead(200, { "Content-Type": "text/html" });
        //3. res.end()를 사용해서 login 페이지에 data.id, data.pw를 출력하세요.
        res.end(`<h1>Login Information</h1><p>ID: ${data.id}</p><p>Password: ${data.pw}</p>`);
      });
    }
  })
